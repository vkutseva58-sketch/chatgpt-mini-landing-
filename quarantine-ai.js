...
const qbtn=$('#questionBtn'),nameEl=$('#personName'),passportNoEl=$('#passportNo'),dialogue=$('#dialogue'),tools=$('.qc-tools');
if(!qbtn||!nameEl||!dialogue||!tools)return;
let activeCase='',usedFor='',busy=false;

function text(sel){return ($(sel)?.textContent||'').trim()}
function caseKey(){return text('#passportNo')||text('#personName')}
...
function refresh(){const n=text('#personName'),key=caseKey();activeCase=key;const valid=n&&n!=='—';ai.disabled=!valid||qbtn.disabled||usedFor===key||busy;const label=document.querySelector('.qc-person-copy>small');if(valid&&!live.isConnected)label?.appendChild(live);if(!valid&&live.isConnected)live.remove()}
...
async function ask(){const question=input.value.trim();const c=context(),key=c.passportNo||c.name;if(busy||question.length<2||!c.name||c.name==='—')return;if(qbtn.disabled){setState('Этот пассажир уже был допрошен.',true);refresh();return}
 busy=true;send.disabled=true;ai.disabled=true;usedFor=key;setState('Пассажир думает над ответом…');
 ...
 try{
  ...
  if(caseKey()===key){dialogue.textContent='— '+answer;$('#interviewBox')?.classList.add('alert');if(navigator.vibrate)navigator.vibrate(12)}
 ...
 }catch(e){if(caseKey()===key)dialogue.textContent='— '+fallback(c,question);setState('Нет связи с AI-каналом. Использован резервный ответ.',true);setTimeout(()=>panel.classList.remove('show'),1150)}
...
const onCaseChange=()=>{const key=caseKey();if(key!==activeCase){activeCase=key;panel.classList.remove('show')}refresh()};
new MutationObserver(onCaseChange).observe(nameEl,{childList:true,characterData:true,subtree:true});
if(passportNoEl)new MutationObserver(onCaseChange).observe(passportNoEl,{childList:true,characterData:true,subtree:true});
